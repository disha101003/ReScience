...
class AttentionHead(nn.Module):
    ...
    def forward(self, x,c):
        ...
        if(c==1):
            attention_probs = attention_probs + torch.randn(attention_probs.size()).to(const.device)*const.std + const.mean
...
class D_buffer:
  ...
  def update(self, task_sem_mem_list,task_num):
    ...
    num_needed_per_class = int(self.max_length / (((task_num + 1)*const.num_classes) / self.tasks))
    self.buffer_images = []
    self.buffer_labels = []

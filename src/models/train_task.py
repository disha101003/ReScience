...
def train_task(
    ...
    for epoch in range(const.epochs):
        ...
        for batch_idx, batch in enumerate(tqdm(
                (trainloader), desc=f"Epoch {epoch + 1} / \
                    {const.epochs} in training", leave=False)):
            x, y = batch
            x, y = x.to(const.device), y.to(const.device)
            # get output from g()
            y_hat_g, _ = model_g(x, c)
            # create a detached copy to be processed further
            y_hat_temp_copy = y_hat_g.detach().clone()
            ...
            # controls sampled from a normal distribution to control different
            # noises introduced in BiRT
            alpha_t_comp, alpha_a_comp, alpha_s_comp, alpha_e_comp = \
                np.random.uniform(0, 1, 4)

            # sampling mini batch from episodic memory
            if not sem_mem.is_empty():
                ...
                if (alpha_t_comp < const.alpha_t):
                    num_change = int(
                        const.percentage_change / 100 * const.batch_size)
                    indices_change_r_y = torch.randperm(
                        len(y))[:num_change].to(const.device)
                    r_y_changed = torch.randint(
                        0, const.num_classes, (num_change,)).to(
                        const.device)
                ...
                if (alpha_s_comp < const.alpha_s):
                    r_y_semantic = r_y_semantic + \
                        torch.rand(r_y_semantic.size()).to(const.device) * \
            ...
            # computing loss
            if (task_index == 0):
                loss_representation = criterion(
                    y_working, y)   # representation loss
            else:
                loss_representation = criterion(
                    y_working, y) + const.alpha_loss_rep * \
                    criterion(model_f_w(r, c)[0], r_y)  # loss

            if (task_index == 0):
                loss_consistency_reg = const.beta_1_loss * \
                    torch.norm(y_working - y_semantic, p=2)**2
            else:
                # consistency regulation noise is also added
                loss_consistency_reg = const.beta_1_loss * torch.norm(
                    y_working - y_semantic, p=2)**2 + const.beta_2_loss * \
                    torch.norm(r_y_working - r_y_semantic, p=2)**2

            loss = loss_representation + const.rho_loss_cr * \
                loss_consistency_reg   # total loss
            loss = loss / const.accum_iter
            loss.backward(retain_graph=True)

            if ((batch_idx + 1) % const.accum_iter ==
                    0) or (batch_idx + 1 == len(trainloader)):
                optimizer.step()
                optimizer.zero_grad()
            ...
            if (alpha_e_comp < const.alpha_e and task_index > 0):
                for params1, params2 in zip(
                        model_f_s.parameters(), model_f_w.parameters()):
                    interpolated_params = const._gamma * \
                        params1.data + (1 - const._gamma) * params2.data
...
if __name__ == '__main__':
    ...
    sem_mem = train_task(const.task_num,
                         task_list[const.task_num],
                         sem_mem,
                         model_g,
                         model_f_w,
                         model_f_s,

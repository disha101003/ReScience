...
SUBSET = 0
# NUM_SUBSET_IMAGES = num_of images in the subset per class for training
# when subset is 1
...
config = {
    ...
    # 4 * hidden_size  (to be also changed when hidden size is changed)
...
alpha_s = 0.005   # controls amount of trial to trial variability,
# by applying noise to logits of semantic memory
...
# hyperparameter for normal distribution used in creating noise to be
# appliedsemntic memory logits
std = 1
# hyperparameter for normal distribution used in creating noise to be
# appliedsemntic memory logits
mean = 0
